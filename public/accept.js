...
async function searchRide() {
  ...
  for(let ride of result){
    let btnName = ride.acceptRide ? 'Accepted' : 'Accept'
    btnName = ride.requestRide ? btnName : 'No request till now'
    template += `<div class="card-deck">
          <div class="card bg-primary">
            <div class="card-body text-center">
              ...
              <button class="btn btn-primary stretched-link" onclick="acceptRide('${ride.uid}')">${btnName}</button>
            </div>
          </div>
      </div><br><br>`
  }
...
console.log(id);
await firebase.firestore().collection('bookings').doc(id).update({acceptRide:true})
